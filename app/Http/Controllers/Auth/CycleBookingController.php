...
//            ->get();
    ...
    public function admin_cycle_reservation_list()
    {   // have to change according to admin
        $list = CycleInfo::join('cycle_availabilities as ca','ca.cycle_id','cycle_infos.id')
            ->join('Users as u','u.id','ca.user_id')
            ->join('cycle_availability_statuses as cas','cas.id','ca.cycle_availability_status_id')
            ->where('ca.cycle_availability_status_id',2)
            ->where('ca.user_id','!=',null)
            ->where('ca.owner_id',auth()->id())
            ->select('ca.id as cycle_availability_id','cycle_infos.brand as brand','cycle_infos.type as type','cycle_infos.model as model',
                'cycle_infos.sku as sku','cycle_infos.cycle_status_id as cycle_info_status',
                'ca.cycle_availability_status_id as cycle_availability_status','ca.available_date as available_date',
                'ca.available_hours as available_hours','u.name as name','u.email as email','cas.name as status_name')
            ->get();
